...
public class AttachmentController {
    ...
    public ResponseEntity<AttachmentDto> uploadAttachment(@RequestParam("file") MultipartFile file,
                                                          ...
                                                          Principal principal) {
        AttachmentDto dto = attachmentService.uploadAttachment(file, ticketId, principal.getName());
        return new ResponseEntity<>(dto, HttpStatus.CREATED);
    }

    @GetMapping("/{id}")
    public ResponseEntity<byte[]> downloadAttachment(@PathVariable Long id, Principal principal) {
        Attachment attachment = attachmentService.getAttachmentById(id, principal.getName());
    ...
    public ResponseEntity<List<AttachmentDto>> getAttachmentsByTicketId(@PathVariable Long ticketId,
                                                                        Principal principal) {
        List<AttachmentDto> list = attachmentService.getAttachmentsByTicketId(ticketId, principal.getName());
